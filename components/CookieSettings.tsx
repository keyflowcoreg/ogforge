...
const CATEGORIES: CategoryInfo[] = [
  {
    ...
    description: "Essential for the website to function. These cookies enable core features such as security, authentication, and accessibility. They cannot be disabled.",
    locked: true,
  },
  {
    id: "analytics",
    label: "Analytics",
    description: "Help us understand how visitors interact with the website by collecting and reporting information anonymously (e.g. Google Analytics).",
    locked: false,
  },
  {
    id: "marketing",
  ...
  },
  {
    id: "preferences",
    label: "Preferences",
    description: "Allow the website to remember choices you make (such as language, region, or display settings) and provide enhanced, personalised features.",
    locked: false,
  },
];
    ...
    >
      ...
      >
        <div className="sticky top-0 z-10 border-b border-zinc-800 bg-zinc-900/95 px-6 py-5 backdrop-blur-sm">
          ...
          <p className="mt-1 text-sm text-zinc-400">Choose which cookie categories you allow. You can change these settings at any time.</p>
        </div>
          ...
          {CATEGORIES.map((cat) => (
            <div key={cat.id} className="flex gap-4 py-5">
              <div className="flex-1 space-y-1">
                <label htmlFor={`toggle-${cat.id}`} className="block text-sm font-medium text-white">
                  {cat.label}
                  {cat.locked && (
              ...
              <div className="flex items-start pt-0.5">
                <Toggle id={`toggle-${cat.id}`} checked={state[cat.id]} disabled={cat.locked} onChange={() => toggle(cat.id)} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="sticky bottom-0 flex justify-end gap-3 border-t border-zinc-800 bg-zinc-900/95 px-6 py-4 backdrop-blur-sm">
          <button type="button" onClick={onClose} className="rounded-lg border border-zinc-700 px-5 py-2.5 text-sm font-medium text-zinc-300 transition-colors hover:border-zinc-500 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-500">
            Cancel
          </button>
          <button type="button" onClick={() => onSave(state)} className="rounded-lg bg-white px-5 py-2.5 text-sm font-medium text-zinc-900 transition-colors hover:bg-zinc-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white">
            Save Preferences
          </button>
        </div>
      </motion.div>
